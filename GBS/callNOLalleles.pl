open $incombo,'<',"@ARGV[0]/NOLCombinedGrep.out";
open $outcombo,'>',"@ARGV[0]/NOLalleles.out";
while ($read1=<$incombo>)
{
@rdprts=split /\t/,$read1;
$chcnt=`agrep -1 $rdprts[0] @ARGV[0]/NOLCombinedGrep.out|grep Grand |cut -d' ' -f3`;
chomp $chcnt;
if($chcnt==2)
{
$existchk=`grep -ch $rdprts[0] @ARGV[0]/NOLalleles.out`;
chomp $existchk;
if($existchk == 0)
{
@allelepair=`agrep -1 $rdprts[0] @ARGV[0]/NOLCombinedGrep.out`;
$Grand=pop @allelepair;
